...
`summarze` function. Some of the functions below should
be self explanatory (like `Mean`, `Std. Dev.`,  `Min`, and `Max`). We can also see that `Obs` gives the number of observations.
...
Before we do this, we need to discuss the concept of `preserve` in Stata.  In Stata, you can only work with one dataset at a time.  We want to work with a sample from the full `ames` dataset, but we would also like to return to the full dataset later on.  Therefore, we will `preserve` the full `ames` dataset before moving forward.  

```{r preserve}
preserve
```

Remember, we can also always read back in the original `ames.dta` dataset if we want to return to the full data.  However, the `preserve` command makes this a bit easier.
...
1.  Take a second sample, also of size 50. How does the 
    ...
    would provide a more accurate estimate of the population mean?  **Hint:** Use the `restore` function to return to the original, full dataset before sampling again.
...
sample mean of each. Note that we use `bootstrap` instead of `sample` to sample with replacement and compute the sample mean of each sample,  since sampling distributions are constructed by sampling
with replacement.
...
Here we use Stata to take 15,000 different samples of size 50 from the population, calculate 

...
set into Stata with the following command.
...
the standard error and construct the interval by hand, or use the `proportion` function.
...
1.  Using the `proportion` function, calculate confidence intervals for the 
    ...
    then use these data sets in the `proportion` function to construct the confidence 
    intervals.
...
variable across the levels of the explanatory variable. This means, in the `proportion` function
we need to specify both variables, response as `y` and explanatory as `x`.

```{r eval=FALSE}
proportion response_var, by(explanatory_variable)
```

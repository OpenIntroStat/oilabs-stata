...
We can visualize these counts by creating a barplot.

```{r smoke-barplot, eval=FALSE}
qplot(x = smoke100, data = cdc)
```

Unlike the scatter or line plot from the Intro to R lab, here we only have one
variable that we want to visualize, `smoke100`, so we don't fill in the `y =` 
argument. `qplot()` makes a smart guess at the type of plot that you want based
on the data you've given it. If you want to be more explicit, you can specify
the geometry that you'd like the data to take.

```{r smoke-barplot, eval=FALSE}
qplot(x = smoke100, data = cdc, geom = "bar")
```

Which should generate the exact same plot.

2.  Calculate numerical summaries for `height` and `age`, and compute the 

...
knitr::opts_chunk$set(eval = F, engine = "stata", engine.path = statapath, comment = "")
...
This lab assumes that you have already installed Stata.  Stata is available for purchase at <www.stata.com>.  You may also wish to see whether your university has Stata licenses available for your use.
...
![](more/stata-interface-2017-mac.pdf)
<br>

You will notice several areas of the screen.  The bottom rectangle is the *Command window*, where you can enter Stata code interactively (or ``on the fly'') that you wish to run.  The results will display in the *Results window* above the command line.  When you open Stata, the Results window displays information about the Stata program you are running.  

The information on the right summarizes information about what information, for example data, we have to work with in Stata. Currently, we do not have any data or information loaded into Stata, so that area is blank.   
...
You will notice that on the opening screen, it said to update.
...
In Stata, you can enter code interactively ("on the fly") in the Command window, or save it in a separate file called a do file.  It is always better to save code in a separate do file because

1. You can save your code cleanly.
2. You can annotate your code with comments explaining what you were trying to do.
3. You can share your work with others.

In these labs, you should always use a do file to record any code that you create.  To create a new do file, select the button at the top for a new do file editor:

...
```{r hist-dep-delay2, eval = T, echo = F, results="hide"}
use "nycflights.dta"
histogram dep_delay
graph export histdelay.png
```
...
```{r hist-dep-delay-bins15a}
histogram dep_delay, bin(15)
```

```{r hist-dep-delay-bins15b, eval = T, echo = F, results="hide"}
use "nycflights.dta"
histogram dep_delay, bin(15)
graph export histdelay15.png
```
...
```{r hist-dep-delay-bins150a}
histogram dep_delay, bin(150)
```

```{r hist-dep-delay-bins150b, eval = T, echo = F, results="hide"}
use "nycflights.dta"
histogram dep_delay, bin(150)
graph export histdelay150.png
```
...
```{r lax-flights-hist2, eval = T, echo = F, results="hide"}
use "nycflights.dta"
histogram dep_delay if dest == "LAX"
graph export lax.png
```
...
We use `bysort` to first sort the data by `origin` to ensure that all of the same origin flights are grouped together, and then apply summarize by each origin.
...
the three airports using a bar plot.
...
```{r bardel, eval = T, echo = F, results="hide"}
use "nycflights.dta"
generate dep_type = 1 if dep_delay < 5
replace dep_type = 0 if dep_delay >= 5
graph bar, over(dep_type) over(origin)
graph export depbar.png
```
...
```{r scatterdel, eval = T, echo = F, results="hide"}
use "nycflights.dta"
generate dep_type = 1 if dep_delay < 5
replace dep_type = 0 if dep_delay >= 5
scatter arr_delay dep_delay if carrier == "AA" | carrier == "DL" | carrier == "UA"
graph export scatterdelay.png
```
